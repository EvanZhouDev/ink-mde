...
const lazyResolvers = [
  ...
  async ([state], compartment) => {
    if (state().options.interface.images) {
      const { images } = await import('./images-xl1iIxYa.js');
  ...
  async ([state], compartment) => {
    ...
    if (tab || shiftTab) {
      const { indentWithTab } = await import('./indentWithTab-yzP9bxnx.js');
  ...
  async ([state], compartment) => {
    if (state().options.placeholder) {
      const { placeholder } = await import('./placeholder-BlHzsP5D.js');
  ...
  async ([state], compartment) => {
    if (state().options.interface.readonly) {
      const { readonly } = await import('./readonly-QuDRaVs2.js');
  ...
  async ([state], compartment) => {
    if (state().options.search) {
      const { search } = await import('./search-4vbOUsSl.js');
  ...
  async ([state], compartment) => {
    if (state().options.interface.spellcheck) {
      const { spellcheck } = await import('./spellcheck-_g8GlJ9M.js');
  ...
  async ([state], compartment) => {
    if (state().options.vim) {
      const { vim } = await import('./vim--tjxVtZp.js');
...
const modules = new Proxy({}, {
  get: (target, prop, _receiver) => {
    ...
    return target[prop] = (async () => {
      const { importer } = await __variableDynamicImportRuntimeHelper((/* #__PURE__ */ Object.assign({"./importers/katex.ts": () => import('./katex-MJDMbj-h.js')})), `./importers/${prop}.ts`);
...
const _tmpl$$7 = ["<div", " class=\"ink-mde-readability\"><span>", "</span></div>"],
  _tmpl$2$2 = ["<span", ">&nbsp;|</span>"],
...
const _tmpl$$6 = ["<span", ">uploading files...</span>"],
...
const _tmpl$$5 = ["<div", ' class="cm-editor"><div class="cm-scroller"><div class="cm-content" contenteditable="true"><div class="cm-line"><br></div></div></div></div>'];
const Editor = (props) => {
  {
...
const _tmpl$$4 = ["<button", " class=\"ink-button\" type=\"button\">", "</button>"];
const Button = props => {
...
const _tmpl$$3 = ["<svg", " viewBox=\"0 0 20 20\" fill=\"none\" stroke=\"currentColor\" stroke-miterlimit=\"5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M6 4V10M6 16V10M6 10H14M14 10V4M14 10V16\"></path></svg>"],
...
const _tmpl$$1 = ["<div class=\"ink-mde-editor\">", "</div>"];
const Root = props => {
  const [state, setState] = useStore();
...
const _tmpl$ = ["<div", ' class="ink-mde-textarea"></div>'];

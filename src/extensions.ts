...
export const resolvers: InkInternal.ExtensionResolvers = [
	...
	([state]: InkInternal.Store) => {
		const isDark =
			state().options.interface.appearance === appearanceTypes.dark;
		const isAuto =
			state().options.interface.appearance === appearanceTypes.auto;
...
export const lazyResolvers: InkInternal.LazyExtensionResolvers = [
	...
	async (
		...
		if (state().options.placeholder) {
			const { placeholder } = await import(
				"/src/vendor/extensions/placeholder"
			);
	...
	async (
		...
		const { markdownKeymap } = await import(
			"/src/vendor/extensions/markdownKeymap"
		);
